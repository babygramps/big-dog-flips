...
- Before every commit, run `npm test` and then `npm run build`. Both must pass on the final changes being committed, including documentation-only changes. No additional user request is needed.
- If either command fails, fix the failure and rerun both before committing. Do not skip the checks or rely on an earlier run after making further changes. Report any unresolved failure instead of committing.
- Follow `docs/testing.md` for test structure and conventions.
...
- Report test and build results when committing. These checks do not require starting a server or visually reviewing the UI.

## Commands

- `npm install` — install dependencies
- `npm test` — run all unit and integration tests; required before every commit
- `npm run build` — build for production; required after tests pass and before every commit
...
- Each player gets `league_settings.points_per_player` votes per round, default 3. All three may be upvotes, or one may be a downvote worth -1 point. Upvotes and downvotes share the same total allowance, with at most one downvote per player. Upvotes may be stacked on one song; a song cannot receive both kinds from the same player. The opposite arrow undoes an existing vote. Ballot progress counts absolute points spent.
...
- Songs are anonymous during voting. Comment names use a per-round alias from `src/lib/anonymousNames.js`, and every comment photo is the shared detective dog at `public/anonymous-commenter.webp` (`ANONYMOUS_COMMENT_AVATAR_URL`). Appreciation reveals song submitters, comment names, and each author's profile photo.
...
During voting, comment photos and the voting alias card all use the single shared detective dog image, so photos cannot reveal authors. From appreciation onward, comment photos use `players.avatar_url`, with the same default image as the profile when no photo is set. Do not assign per-player anonymous photos or cycle them per round. A requested new dog photo should update the player's profile photo so it changes everywhere. Real, non-AI dog photographs are bundled in `public/anonymous-dogs/` with source credits.
