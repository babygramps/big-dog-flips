...
- Songs are anonymous during voting. Comment names use a per-round alias from `src/lib/anonymousNames.js`, and every comment photo is the shared McGruff image at `public/anonymous-commenter-v2.webp` (`ANONYMOUS_COMMENT_AVATAR_URL`). Appreciation reveals song submitters, comment names, and each author's profile photo.
